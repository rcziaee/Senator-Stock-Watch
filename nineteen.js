...
const generateTest = async () => {
  ...
  for (i = 0; i < data.length; i++) {
    if ((data[i].transaction_date.slice(-4)) === '2019') {
...
setTimeout(() => {
  ...
  const nineteentext = document.querySelector("#nineteentext");
  let nineteentextprep = ""
  for (trade of twenty_nineteen) {
    nineteentextprep += '<blockquote> Senator: ' + trade.senator + ' <br> Transaction Date: ' + trade.transaction_date + '<br> Amount: ' + trade.amount + '<br> Asset Type: ' + trade.asset_type + '<br> Ticker: ' + trade.ticker + '<br> Owner: ' + trade.owner + '</blockquote>';
    //console.log(test);
  }

  nineteentext.innerHTML = nineteentextprep;
