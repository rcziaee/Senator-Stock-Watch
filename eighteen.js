...
const generateTest = async () => {
  ...
  for (i = 0; i < data.length; i++) {
    if ((data[i].transaction_date.slice(-4)) === '2018') {
...
setTimeout(() => {
  ...
  const eighteentext = document.querySelector("#eighteentext");
  let eighteentextprep = ""
  for (trade of twenty_eighteen) {
    eighteentextprep += '<blockquote> Senator: ' + trade.senator + ' <br> Transaction Date: ' + trade.transaction_date + '<br> Amount: ' + trade.amount + '<br> Asset Type: ' + trade.asset_type + '<br> Ticker: ' + trade.ticker + '<br> Owner: ' + trade.owner + '</blockquote>';
  }

  eighteentext.innerHTML = eighteentextprep;
